...
本次实验旨在开发并评估多种循环神经网络（RNN）架构在文本分类任务中的表现。通过构建传统RNN、门控循环单元（GRU）、长短期记忆网络（LSTM）、双向RNN（Bi-RNN, Bi-GRU, Bi-LSTM）以及Transformer模型，系统地比较了这些模型在Yelp2013数据集上的分类准确率。

### 实验结论：

1. **模型性能对比**：
   - 在不同的RNN架构中，中等规模的数据集上，**Bi-LSTM**表现最为优异，达到了最高的分类准确率（Accuracy）**0.661**。
...
本实验通过系统性的比较，验证了不同RNN架构在文本分类任务中的有效性。结果表明，**双向LSTM**在捕捉长距离依赖和上下文信息方面具有明显优势，能够显著提升分类准确率。此外，合理选择Pooling方法和文本长度也是优化模型性能的重要因素。
